...
export async function POST(req: Request) {
  try {
    ...
    const { storeName, contactEmail, currency, taxRate, flatShippingFee, ordersEnabled } = body;
    ...
    if (existing.length === 0) {
      ...
      const updated = await db.update(storeSettings).set({
        ...
        taxRate: Number(taxRate),
        flatShippingFee: Number(flatShippingFee),
        ordersEnabled: ordersEnabled !== undefined ? Boolean(ordersEnabled) : existing[0].ordersEnabled,
        updatedAt: new Date(),
